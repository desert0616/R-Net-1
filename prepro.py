...
def encode_pos(config):
    filename = config.train_file
    pos_tag_set = set()

    with open(filename, "r") as fh:
        source = json.load(fh)
        for article in source["data"][:5]:
            for para in article["paragraphs"]:
                context = para["context"].replace("''", '" ').replace("``", '" ')
                context_tokens = word_tokenize(context)
                for _, pos_tag in nltk.pos_tag(context_tokens):
                    pos_tag_set.add(pos_tag)
    pos2idx_dict = dict()
    pos2idx_dict['UNKNOWN'] = 0
    for idx, tag in enumerate(pos_tag_set):
        pos2idx_dict[tag] = idx + 1
    return pos2idx_dict
...
def prepro(config):
    ...
    pos2idx_dict = encode_pos(config)

    word_counter, char_counter = Counter(), Counter()
    train_examples, train_eval = process_file(
        config.train_file, "train", word_counter, char_counter, pos2idx_dict)
    dev_examples, dev_eval = process_file(
        config.dev_file, "dev", word_counter, char_counter, pos2idx_dict)
    test_examples, test_eval = process_file(
        config.test_file, "test", word_counter, char_counter, pos2idx_dict)
